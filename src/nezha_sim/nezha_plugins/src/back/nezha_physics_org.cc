...
)
{
  ...
  void Hydrodynamics::UpdateSubmergedTriangles()
  {
    ...
    gzmsg << "=== nasv_physics.cc:Mesh Statistics ===" << std::endl;
    gzmsg << "Total triangles:     " << this->data->triangleProperties.size() << std::endl;
    gzmsg << "Submerged triangles: " << this->data->submergedTriangles.size() << std::endl;
    gzmsg << "Reduction ratio:     " 
          << (1.0 - (double)this->data->submergedTriangles.size() / 
                    this->data->triangleProperties.size()) * 100.0 
          << "%" << std::endl;
  }
...
const double MIN_AREA = 0.005;  // 鍘熷€�: 5.0e-6 (0.000005 m虏)
                                 // 鏂板€�: 0.001 m虏 (10 cm虏)
                                 // 鍙互灏濊瘯: 0.005, 0.01, 0.05
...
const double MAX_ASPECT_RATIO = 15.0; 
...
const double MIN_HEIGHT_DIFF = 0.01; 
...
const double MIN_SUBMERGED_AREA = 0.001;  // 鍘熷€�: 5.0e-6
                                           // 鏂板€�: 0.001 m虏
...
const double MIN_VERTEX_DISTANCE = 0.01; 
...
const double MIN_SUBMERGED_AREA = 0.001;  // 鍘熷€�: 1.0e-8
                                           // 鏂板€�: 0.001 m虏
  ...
  void Hydrodynamics::ComputeBuoyancyForce()
  {
    ...
    this->data->torque += sumTorque;
        gzmsg << "=== Buoyancy Force ===" << std::endl;
    gzmsg << "Force Z: " << sumForce.z() << " N" << std::endl;
    gzmsg << "Submerged area: " << this->data->submergedArea << " m虏" << std::endl;
  }
