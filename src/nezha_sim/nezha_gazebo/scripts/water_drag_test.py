...
class ZOverLController:
    def __init__(self):
        # Constant: total vehicle length L
        self.L = 0.323  # meters
        ...
        self.target_zol_high = rospy.get_param("~target_z_over_l_high", 1.0)  # fully out of water
        self.target_zol_low  = rospy.get_param("~target_z_over_l_low", 0.0)   # fully submerged
        ...
        # Try to connect to the transmedia phase/water-surface service
        ...
        # Initial target: move up to high
    ...
    def query_phase_sample(self):
        ...
        try:
            resp = self.cli_phase()
            # Prefer the surface_z and z_over_l provided by the service
    ...
    def estimate_z_over_l(self, z_model):
        # Estimate the top height: z_top = z_model + z_top_offset
    ...
    def compute_command_from_zol(self, zol_now):
        ...
        # Direction: larger target => need to move up (z increases) => negative velocity; smaller target => move down => positive velocity
        cmd_speed = -self.speed if dzol > 0.0 else self.speed

        # Proportional scaling in the near zone (linear scale-down within 10*tol, minimum 10%)
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            # Prefer the z_over_l provided by the service
            ...
            if ok and z_over_l is not None:
                ...
                # fallback: rely only on the model z and configured parameters
