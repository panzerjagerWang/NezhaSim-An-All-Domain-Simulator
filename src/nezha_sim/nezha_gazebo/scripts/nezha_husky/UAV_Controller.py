...
class UAVController:
    ...
    def __init__(self, debug=False):
        ...
        self.throttle_pub = rospy.Publisher('/nezha_husky/command/throttle_percent', Float32, queue_size=1)
        # Subscribe to motor status information
        rospy.Subscriber("/nezha_husky/command/motor_speed", Actuators, self.motor_speed_callback)

        # Publish control commands
        self.pose_cmd_pub = rospy.Publisher("/nezha_husky/command/pose", PoseStamped, queue_size=10)
        self.trajectory_pub = rospy.Publisher("/nezha_husky/command/trajectory", MultiDOFJointTrajectory, queue_size=10)
    ...
    def _check_connections(self):
        ...
        topics = ['/nezha_husky/ground_truth/pose', '/nezha/command/pose', '/nezha/command/trajectory']
        ...
        for topic in topics:
            ...
            if info:
                ...
                rospy.logwarn(f"Topic {topic} is not publishing anything")

    def pose_callback(self, msg):
        """Receive /nezha_husky/ground_truth/pose messages and update the current z and yaw."""
        self.current_pose = msg
        self.current_z = msg.position.z
        # Convert quaternion to Euler angles (only yaw is needed)
    ...
    def motor_speed_callback(self, msg):
        """Receive motor status information."""
    ...
    def waypoint_navigation(self, target_position):
        ...
        if self.current_pose is None:
            rospy.logwarn("Cannot navigate: current pose unknown")
            return False
        ...
        if waypoint_error > 0.1:  # Apply control when the target error exceeds the set threshold
            self.move_to_waypoint(target_position)
            rospy.loginfo("Navigating to waypoint: target error=%.2f m", waypoint_error)
            return False
        else:
            rospy.loginfo("Reached waypoint, target error=%.2f m", waypoint_error)
            return True
    ...
    def set_target_height(self, height):
        ...
        if self.current_pose is not None:
            ...
            rospy.logwarn("Cannot set altitude: current position unknown")
            return False

    def maintain_height(self):
        """
        Altitude hold: keep the UAV at the target altitude.
        """
        if self.current_z is None:
            rospy.logwarn("Cannot maintain altitude: current altitude unknown")
            return False

        height_error = self.target_height - self.current_z
        if abs(height_error) > 0.1:
            # Use position control to adjust altitude
            if self.current_pose:
                target_position = Pose()
                target_position.position.x = self.current_pose.position.x
                target_position.position.y = self.current_pose.position.y
    ...
    def takeoff(self):
        """
        Takeoff: command the UAV to climb to the target altitude.
        """
        ...
        while self.current_pose is None:
            if rospy.Time.now().to_sec() - start_time > timeout:
                rospy.logwarn("Timed out waiting for position information")
                return False
        ...
        rospy.loginfo(f"Current altitude: {self.current_z:.2f}m, climbing to target altitude {self.target_height:.2f}m...")
    ...
    def land(self):
        ...
        if self.current_pose is None:
            rospy.logwarn("Cannot land: current position unknown")
            return False

        # Create the landing target position (same as current position but at altitude 0)
        landing_target = Pose()
...
def main():
    ...
    # Check whether the UAV has received position information
    timeout = 20  # 20-second timeout
    start_time = rospy.Time.now().to_sec()
    while uav.current_pose is None:
        if rospy.Time.now().to_sec() - start_time > timeout:
            rospy.logerr("Unable to obtain UAV position information; please check whether the topic /nezha_husky/ground_truth/pose is being published normally")
            return
        rospy.loginfo("Waiting to receive UAV position information...")
        rospy.sleep(1.0)

    # Set a low takeoff altitude for testing
    ...
    rospy.loginfo("Starting navigation to waypoint (1.0, 0.0, 2.0)...")

    # Loop until the target is reached or the timeout elapses
    ...
    if reached:
        ...
        rospy.logwarn("Navigation timed out, waypoint not reached")
    ...
    rospy.loginfo("Returning to start point (0.0, 0.0, 2.0)...")
    target.position.x = 0.0
    target.position.y = 0.0

    # Loop until the start point is reached or the timeout elapses
    timeout = 30  # 30-second timeout
    start_time = rospy.Time.now().to_sec()
    reached = False
...
def main():
    ...
    max_attempts = 0   # 0 means retry indefinitely; change to a positive integer to limit the count
    attempt = 0
    ...
    while not takeoff_success and (max_attempts == 0 or attempt < max_attempts) and not rospy.is_shutdown():
        ...
        if not takeoff_success:
            ...
            rospy.sleep(3.0)
    if not takeoff_success:
        rospy.logerr("Takeoff still failed after multiple attempts, exiting.")
        return

    rospy.loginfo("Takeoff succeeded, waiting 5 seconds...")
    rospy.sleep(5.0)

    # The following flow matches the original logic...
    target = Pose()
    target.position.x = 1.0
    target.position.y = 0.0
