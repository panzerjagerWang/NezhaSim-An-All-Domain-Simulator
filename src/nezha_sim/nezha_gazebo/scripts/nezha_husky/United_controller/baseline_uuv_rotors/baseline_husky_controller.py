...
# Matplotlib配置 (headless for batch baseline runs)
import matplotlib
matplotlib.use('Agg')
...
class GlobalDataRecorder:
    ...
    def initialize_force_services(self):
        """BASELINE: subscribe to the genuine UUV_simulator plugin debug-force
        topics (no XXSim get_hydrodynamics_forces / get_phase_sample services,
        no PSC/TR/NSE)."""
        ...
        self._latest = {
            ...
            'added_coriolis': (0.0, 0.0, 0.0),
            'is_submerged': False,
        }

        def _wcb(msg, key):
            f = msg.wrench.force
            self._latest[key] = (f.x, f.y, f.z)

        base = '/debug/forces/nezha_husky_bl/base_link'
        for comp in ('restoring', 'damping', 'added_mass', 'added_coriolis'):
            rospy.Subscriber(base + '/' + comp, WrenchStamped, _wcb,
                             callback_args=comp, queue_size=1)
        rospy.Subscriber('/nezha_husky_bl/is_submerged', Bool,
                         lambda m: self._latest.__setitem__('is_submerged', bool(m.data)),
                         queue_size=1)
        self.get_forces_service = True       # mark "available"
        self.get_phase_service = True
        print("✓ 已订阅 genuine UUV 力调试话题 (/debug/forces/nezha_husky_bl/base_link/*)")

    def get_wave_surface(self):
        """BASELINE: flat lake surface (z=0); phase from the UUV is_submerged flag."""
        sub = getattr(self, '_latest', {}).get('is_submerged', False)
        return {
            'surface_z': 0.0,
            'z_over_l': 0.0,
            'phase_name': 'SUBMERGED' if sub else 'ABOVE',
        }
    ...
    def get_forces(self):
        """BASELINE: latest genuine UUV_simulator wrench components from the
        /debug/forces topics (wave force is 0 -- UUV has no wave coupling)."""
        if not hasattr(self, '_latest'):
            return None
        r = self._latest['restoring']
        d = self._latest['damping']
        a = self._latest['added_mass']
        c = self._latest['added_coriolis']
        return {
            'buoyancy_x': r[0], 'buoyancy_y': r[1], 'buoyancy_z': r[2],
            'damping_x': d[0], 'damping_y': d[1], 'damping_z': d[2],
            'wave_x': 0.0, 'wave_y': 0.0, 'wave_z': 0.0,
            'added_mass_x': a[0], 'added_mass_y': a[1], 'added_mass_z': a[2],
            'coriolis_x': c[0], 'coriolis_y': c[1], 'coriolis_z': c[2],
        }
    ...
    def switch_to_robot(self, robot_type):
        ...
        if robot_type == RobotType.UGV:
            ...
            self.image_sub = rospy.Subscriber(
                '/nezha_husky_bl/camera_/image_raw',
                Image,
...
class UAVController(BaseRobotController):
    ...
    def __init__(self, waypoint_file='smoothed_trajectory_data.csv', enable_realtime_plot=True):
        ...
        self.traj_pub = rospy.Publisher(
            '/nezha_husky_bl/command/trajectory',
        ...
        # 订阅器：获取当前位置信息
        rospy.Subscriber(
            '/nezha_husky_bl/ground_truth/odometry',
            Odometry,
            self._odom_callback,
            queue_size=1
        )
...
class UGVController(BaseRobotController):
    ...
    def __init__(self, waypoint_file='smoothed_trajectory_data.csv', enable_realtime_plot=True):
        ...
        # 发布器
        self.cmd_vel_pub = rospy.Publisher('/nezha_husky_bl/cmd_vel', Twist, queue_size=10)

        # 订阅器
        rospy.Subscriber('/nezha_husky_bl/ground_truth/odometry', Odometry, self._odom_callback)
...
class UUVController(BaseRobotController):
    ...
    def __init__(self, waypoint_file='smoothed_trajectory_data.csv', enable_realtime_plot=True):
        ...
        # 订阅odometry获取速度
        rospy.Subscriber("/nezha_husky_bl/ground_truth/odometry", Odometry,
        ...
        # ========== ROS订阅器 ==========
        rospy.Subscriber("/nezha_husky_bl/ground_truth/pose", Pose,
                         self._pose_callback, queue_size=1)

        # ========== ROS发布器 ==========
        self.pub_pitch = rospy.Publisher("/nezha_husky_bl/thrusters/0/input",
                                         FloatStamped, queue_size=10)
        self.pub_left = rospy.Publisher("/nezha_husky_bl/thrusters/1/input",
                                        FloatStamped, queue_size=10)
        self.pub_right = rospy.Publisher("/nezha_husky_bl/thrusters/2/input",
                                         FloatStamped, queue_size=10)
    ...
    def reset_model_to_origin(self, x=0.0, y=0.0, z=0.0, roll=0.0, pitch=0.0, yaw=0.0):
        ...
        try:
            ...
            model_state.model_name = 'nezha_husky_bl'
...
class MultiRobotController:
    ...
    def _reset_to_land_start(self):
        """Teleport the husky to a consistent UGV land start (the spawn can settle
        or drift off the asphalt edge, which stalls the whole mission)."""
        from gazebo_msgs.srv import SetModelState as _SMS
        from gazebo_msgs.msg import ModelState as _MS
        try:
            rospy.wait_for_service('/gazebo/set_model_state', timeout=10)
            _set = rospy.ServiceProxy('/gazebo/set_model_state', _SMS)
            ms = _MS()
            ms.model_name = 'nezha_husky_bl'
            ms.reference_frame = 'world'
            ms.pose.position.x = -8.06
            ms.pose.position.y = 0.0
            ms.pose.position.z = 0.85
            ...
            for _ in range(15):
                ...
                rospy.sleep(0.1)
            rospy.sleep(2.0)
            print("🔄 husky 已复位到陆地起点 (-8.06, 0, 0.85)")
        except Exception as e:
            print(f"⚠️ 复位到起点失败: {e}")
    ...
    def run_single_epoch(self, epoch):
        ...
        # Ensure a deterministic land start before recording / driving.
        self._reset_to_land_start()

        # BASELINE: run every phase with REAL control and ALWAYS save whatever was
        # recorded (even if the genuine-RotorS takeoff or a phase under-performs --
        # that degradation is exactly the baseline result we want to capture).
        # Record a replay rosbag for the whole mission as a child process (so its
        # lifetime is tied exactly to this mission).
        _ns = 'nezha_husky_bl'
        _bag_ts = time.time()
        _bag_path = os.path.join(os.getcwd(), 'baseline_run_%.4f.bag' % _bag_ts)
        _bag_topics = [
            '/%s/is_submerged' % _ns, '/%s/ground_truth/odometry' % _ns,
            '/%s/cmd_vel' % _ns,
            '/%s/thrusters/0/input' % _ns, '/%s/thrusters/1/input' % _ns,
            '/%s/thrusters/2/input' % _ns,
            '/%s/command/trajectory' % _ns, '/%s/command/motor_speed' % _ns,
            '/%s/motor_speed/0' % _ns, '/%s/motor_speed/1' % _ns,
            '/%s/motor_speed/2' % _ns, '/%s/motor_speed/3' % _ns,
        ]
        try:
            _bag_proc = subprocess.Popen(
                ['rosbag', 'record', '--lz4', '-O', _bag_path] + _bag_topics,
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                preexec_fn=os.setsid)
            print('🎥 rosbag 录制开始: %s' % _bag_path)
        except Exception as e:
            _bag_proc = None
            print('⚠️ rosbag 启动失败: %s' % e)

        ok = True
        try:
            # ========== UGV 阶段 (real wheel control) ==========
            print("🚗 阶段 1: UGV 地面行驶")
            if not self._track_with_global_plot_and_record(self.ugv, 'UGV'):
                print("❌ UGV 阶段失败"); ok = False
            print("✅ UGV 阶段完成\n")
            rospy.sleep(2.0)

            # ========== UUV 阶段 (real thruster control) ==========
            print("🌊 阶段 2: UUV 水下航行")
            if not self._track_with_global_plot_and_record(self.uuv, 'UUV'):
                print("❌ UUV 阶段失败"); ok = False
            print("✅ UUV 阶段完成\n")
            rospy.sleep(2.0)

            # ========== UAV 阶段 (real RotorS flight, no NSE) ==========
            print("✈️ 阶段 3: UAV 空中飞行")
            if not self._track_with_global_plot_and_record(self.uav, 'UAV'):
                print("❌ UAV 阶段失败 (genuine RotorS, no NSE)"); ok = False
            print("✅ UAV 阶段结束\n")
        except Exception as e:
            print(f"⚠️ 任务异常: {e}")
            ...
            # 停止 rosbag 录制
            if _bag_proc is not None:
                try:
                    os.killpg(os.getpgid(_bag_proc.pid), signal.SIGINT)
                    _bag_proc.wait(timeout=15)
                    print('🎥 rosbag 录制结束: %s' % _bag_path)
                except Exception:
                    try:
                        os.killpg(os.getpgid(_bag_proc.pid), signal.SIGKILL)
                    except Exception:
                        pass
            # 总是保存已采集数据
            self.data_recorder.save_to_csv(
                f'trajectory_data_with_forces_{time.time()}.csv')
